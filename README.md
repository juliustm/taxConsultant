...
-   **A Scanner That Works With No Signal**: An installable phone app (`/scan/`) that opens straight into a fullscreen QR scanner — no menu, no login screen, camera live in about a second. It is offline-first in the literal sense: with the network switched off it still opens, still scans, still shows history, and queues everything in IndexedDB until connectivity returns. Nothing is loaded from a CDN at runtime. On a phone with several rear cameras the good one is choosable from Diagnostics and remembered, because `facingMode: environment` has no way to say which — and the default is routinely the ultra-wide, the worst of them for a receipt held at arm's length. What the shutter keeps is what the viewfinder was showing, cropped to it rather than to the whole sensor.
-   **The Paper Behind the Figures**: A scan whose QR code the phone reads uploads the photograph it read it from, not just the code. The receipt TRA confirms then has an image anyone can go back to — to check a line item, to see the state the paper was in, to settle an argument about what was bought — and the server's own decoder can be run over an ordinary readable code rather than only over the ones a phone had already failed on.
...
-   **And Know When to Stop Trying**: Rebuilding an address from transcribed text is the most valuable step in that chain when the document really is an EFD receipt, and the most expensive when it is not — a mobile-money SMS carries a plausible run of digits too, so an address gets built that the portal will never confirm and the submission spends its whole retry schedule finding out. So what the model read is kept on the submission from the moment it exists, and shown on its page while the retries run. One button files it as the receipt, correctable field by field, with no second vision call and no further requests to TRA. Instances that collect a lot of documents that are not receipts can switch the rebuilding off altogether.
